...
export default function ActivitiesMentor(props) {
  ...
  const handleFromDateChange = (event, selectedDate) => {
    ...
    handleChange(formatDate(selectedDate), 'start_date', Idx)
  ...
  const handleToDateChange = (event, selectedDate) => {
    ...
    handleChange(formatDate(selectedDate), 'end_date', Idx)
  ...
  const selectOption = (option, Idx) => {
    if (option === 'Yes') {
      handleChange(2, 'demon_by_mentor', Idx)
    } else {
      handleChange(1, 'demon_by_mentor', Idx)
    }
        ...
        <View style={{ height: '85%' }}>
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <View style={{ marginTop: 12, marginHorizontal: 20 }}>
              ...
              >
                {mentorDemoValue(item.demon_by_mentor)}
              </Text>
              {showModal === true && (
                <View
                  style={{
              ...
              <TextInput
                ...
                defaultValue={
                  item?.no_time_demo_mentor === null
                    ? 0
                    : JSON.stringify(item?.no_time_demo_mentor)
                }
                keyboardType="numeric"
                onChangeText={(e) => {
                  const inputValue = e
                  if (/^[0-9]*$/.test(inputValue)) {
                    handleChange(e, 'no_time_demo_mentor', Idx)
                  } else if (inputValue === '') {
                    response.activities[Idx].no_time_demo_mentor = ''
                  } else {
                    e = response.activities[Idx].no_time_demo_mentor
                  }
                  // console.log(e)
                  // console.log(/^[0-9]*$/.test(e))
                }}
              />
              ...
              <TextInput
                style={styles.responseEdit}
                defaultValue={item?.mn_comments}
                multiline
                onChangeText={(e) => handleChange(e, 'mn_comments', Idx)}
              />
            </View>
          </ScrollView>
        </View>
