...
export default function SignUpScreen(props) {
  ...
  useEffect(() => {
    ...
    SignUpScreen.navListener = navigation.addListener('didFocus', () => {
  ...
  async function signUpButtonPressed() {
    ...
    if (!phoneno.test(states.number.trim())) {
      setStates({
        ...states,
      ...
      return;
    } else if (states.number.trim().length != 10) {
      setStates({
        ...states,
        validation: 'Please enter valid phone number',
        status: true,
      ...
      try {
        ...
        if (response.statusCode === 200) {
          let body = {
            ...response.body,
            username: response?.body?.data?.uid,
          };
          setStates({
            ...states,
            spinner: false,
