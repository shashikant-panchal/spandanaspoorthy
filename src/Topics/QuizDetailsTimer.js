...
export default function QuizDetailsTimerScreen(props) {
  ...
  const [timer, setTimer] = useState(qObject.rtime || qObject.oduration)
  ...
  useEffect(() => {
    ...
    QuizDetailsTimerScreen.navListener = navigation.addListener(
      'didFocus',
      () => {
    ...
    return () => {
      unsubscribe()
      clearInterval(QuizDetailsTimerScreen.clockCall)
  ...
  useEffect(() => {
    // Add the back button event listener when the modal is open
    if (retakePopUp) {
  ...
  }, [retakePopUp, finalRetakePopUp]);

  const handleBackButtonPress = () => {
    // Prevent the back button from doing anything while the modal is open
    return true;
  };
  ...
  useEffect(() => {
    ...
    const handleBeforeUnload = async () => {
      await saveTimerInDatabase(timer)
    }
    const handleUnload = () => {
  ...
  async function getQuiz(obj) {
    ...
    try {
      ...
      setCurrentQuestion(
        response?.currentQuestion ? response?.currentQuestion : 0
      )
      setBtnClick(true)
      setQIsLoading(false)
      setSpinner(false)
  ...
  async function syncUserQuizData(score, op, questionsJSON) {
    ...
    if (qtype === 2 || qtype === '2') {
      ...
      //   alert('syncUserQuizData====' + JSON.stringify(body))
      try {
        const response = await API.post(
          config.aws_cloud_logic_custom_name,
          '/syncUserDataWeb',
  ...
  async function saveTimerInDatabase(updatedTimer) {
    ...
    questionsJSON.attempt = attempt
    questionsJSON.qtype = qtype
    questionsJSON.oduration = qObject?.oduration
    questionsJSON.timer = qObject?.timer
    questionsJSON.quiz_length = qObject?.quiz_length
    ...
    try {
      ...
      console.log('updateSummativeQuiz==' + JSON.stringify(response))
    } catch (error) {
      console.error(error)
    }
  }
  ...
  function quizEndFunction() {
    // Alert.alert(" ","Your time is overrrrrrrrrrrrr")
    for (let i = 0; i < quizQuestion.length; i++) {
      if (quizQuestion[i].res === 1) {
        scoreRef.current = scoreRef.current + 1
      } else {
    ...
    let score = quizQuestion.filter((item) => item.answeredOption).length
    if (scoreAchieved(score)) {
      if (qObject.op !== 2) {
        gotScore.current.push(score)
        let questionsJSON = {}
  ...
  function decrementClock() {
    // alert("de")
    if (remMins.current === 0 && remSecs.current === 1) {
      setSpinner(true)
      Alert.alert(' ', 'Your time is over')
      quizEndFunction()
    } else if (remMins.current !== 0 && remSecs.current === 0) {
      remMins.current -= 1
      remSecs.current = 60
    }
    remSecs.current -= 1
    setTimeCheck(remSecs.current)
  }

  function countDowntimer() {
    return (
      <View>
        {remMins.current < 1 && remSecs.current < 1 ? null : (
          <View style={{ alignItems: 'flex-end' }}>
            ...
            >
              {' '}
              {remMins.current < 10 ? 0 : null}
              {remMins.current}:{remSecs.current < 10 ? 0 : null}
              {remSecs.current}{' '}
            </Text>
          </View>
        )}
